...
#   cat /scratch/$USER/datagen/data/processed/preprocessing_rejection_manifest.csv
# ==========================================
...
python preprocess.py \
    --identitydir "$DATA_DIR/identities" \
    --processeddir "$DATA_DIR/processed" \
...
echo "  cat $DATA_DIR/processed/preprocessing_rejection_manifest.csv | cut -d, -f13 | sort | uniq -c | sort -rn"
echo ""
if [ -d "$DATA_DIR/processed/images" ]; then
    ACCEPTED=$(find "$DATA_DIR/processed/images" -type f | wc -l)
